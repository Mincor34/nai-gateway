...
(() => {
  ...
  if (!approved || !VPS_HOST) {
    ...
    let injectGearButton = function() {
      if (document.getElementById("vps-gear-container"))
        return;
      ...
      startStatePolling();
    }, updateProgressRing = function(percent, color, visible = true) {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          if (data.debug_authorized) {
            ...
            debugStatusMsg = `<span style="color:#2ecc71; font-weight:bold;">\u{1F7E2} Authorized by Administrator (${mins}m remaining)</span>`;
